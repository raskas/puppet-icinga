...
class icinga ($pnp4nagios_mode = undef,
  ...
  package {
    'icinga':
      ...
      require => Group['icinga','icingacmd'];
    'icinga-doc':
      ensure  => installed,
  ...
  # We create the groups before the rpm does, otherwise the gid can conflict
  group {
    'icinga':
      gid => '498';
    'icingacmd':
  ...
  file {
    ...
    '/etc/icinga/objects/notifications.cfg':
      ...
      group   => 'icinga',
      mode    => '0664',
      source  => 'puppet:///modules/icinga/notifications.cfg',
      notify  => Service['icinga'],
      require => Package['icinga'];
...
class icinga::nagios-plugins-snmp {
  ...
  nagios_command {
    'check_snmp_int':
      command_line => 'usr/bin/perl \$USER1$/check_snmp_int -H \$HOSTADDRESS$ -C public -n \$ARG1$ -w 0,0 -c 0,0 -r -k -Y -f -B -d 60 \$ARG2$';
    'check_snmp_load':
